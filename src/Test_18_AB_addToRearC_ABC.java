...
public class Test_18_AB_addToRearC_ABC
{
	...
	//****** Constants used in tests *****************
		...
		// Number of tests run 
		private static final int NUM_TESTS = 53; 
		...
		private static final Character MIDDLE = IndexedUnsortedListTests.B;
		...
		// Third valid index inside of list 
		...
		// Fourth valid index where can add a new element 
	...
	public void run(ListType listType)
	{
		...
		try 
		{
			...
			// Test: remove(MIDDLE) *********
			initialize(); 
			result = IndexedUnsortedListTests.remove(list, MIDDLE, Result.MatchingValue); 
			RunIndexedUnsortedTests.printResults("remove(MIDDLE)", result);
			...
			initialize(); 
			result = IndexedUnsortedListTests.contains(list, MIDDLE, Result.True); 
			RunIndexedUnsortedTests.printResults("contains(MIDDLE)", result);
			...
			// Test: addAfter(ELEMENT, MIDDLE)  *************
			initialize(); 
			result = IndexedUnsortedListTests.addAfter(list, ELEMENT, MIDDLE, Result.NoException); 
			RunIndexedUnsortedTests.printResults("addAfter(ELEMENT, MIDDLE)", result);
			...
			// Test: remove(MIDDLE_INDEX) **********
			initialize(); 
			result = IndexedUnsortedListTests.remove(list, MIDDLE_INDEX, MIDDLE, Result.MatchingValue); 
			RunIndexedUnsortedTests.printResults("remove(MIDDLE_INDEX)", result);
			...
			// Test: set(MIDDLE_INDEX, ELEMENT) ********
			initialize(); 
			result = IndexedUnsortedListTests.set(list, MIDDLE_INDEX, ELEMENT, Result.NoException); 
			RunIndexedUnsortedTests.printResults("set(MIDDLE_INDEX, ELEMENT)", result);
			...
			// Test: get(MIDDLE_INDEX) ***************
			initialize(); 
			result = IndexedUnsortedListTests.get(list, MIDDLE_INDEX, MIDDLE, Result.MatchingValue); 
			RunIndexedUnsortedTests.printResults("get(MIDDLE_INDEX)", result);
			...
			// Test: indexOf(MIDDLE) ***********
			initialize(); 
			result = IndexedUnsortedListTests.indexOf(list, MIDDLE, MIDDLE_INDEX, Result.MatchingValue); 
			RunIndexedUnsortedTests.printResults("indexOf(MIDDLE)", result);
			...
			// Test: listIterator(int)
			initialize(); 
			result = IndexedUnsortedListTests.listIterator(list, MIDDLE_INDEX, Result.True); 
